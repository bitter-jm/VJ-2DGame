...
{ 
	...
	if (Game::instance().getCurrentLevel() == 1 && float(posPlayer.x) <= float(SCREEN_WIDTH - 1) / 2.0f) {
		sprite->setPosition(glm::vec2(float(tileMapDispl.x), float(tileMapDispl.y)));
	}
	else sprite->setPosition(glm::vec2(float(tileMapDispl.x + posPlayer.x - 768/2), float(tileMapDispl.y))); // PROJECTION LEVEL 1 & 3
